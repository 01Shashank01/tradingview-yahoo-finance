...
def fetch_yahoo_data(ticker, interval, ema_period=20, indicators=None):
    ...
    if 'rsi' in indicators:
        rsi_values = rsi_calculation(data['Close'], 14)
        data['RSI'] = rsi_values
    ...
    return candlestick_data, indicators_data

@app.route('/')
def index():
    return render_template('index.html')

@app.route('/api/data/<ticker>/<interval>/<int:ema_period>')
def get_data(ticker, interval, ema_period):

    indicators = request.args.get('indicators', '')
    indicators = indicators.split(',') if indicators else []

    candlestick_data, indicators_data = fetch_yahoo_data(
        ticker, interval, ema_period, indicators
    )
